...
```
deepLearning/
├── train.py                  # 训练脚本：MIT-BIH 数据 → RNN/GRU/LSTM 模型
...
训练脚本 [train.py](train.py) 第 96-133 行的 `load_mitbih_data()` 函数展示了完整的本地数据加载流程。
...
每种模型权重独立存储，不会互相覆盖，方便老师切换对比不同架构的预测效果。两个脚本（训练/推理）的路径完全一致。
...
一个 `SleepRNNDemo` 类通过 `cell_type` 参数同时支持 RNN / GRU / LSTM 三种架构，训练和推理脚本中的模型定义完全一致（代码不重复）。
...
| 波形渲染 | D3.js v7 | 150 点滑动窗口 SVG 波形实时绘制 |
...
打开 `MIT-BIH_data/result/` 目录下的训练结果截图，可对比三种模型的 Loss 收敛曲线。
...
| 文件 | 行号 | 改动内容 |
|------|------|----------|
| `train.py` | 1-13 | 新增模块级文档字符串，描述自定义亮点 |
| `train.py` | 29 | 权重路径改为 `MIT-BIH_data\weight\` 独立目录 |
| `train.py` | 51 | **修复 BUG**: `nn.Mod = nn.GRU(...)` → `nn.GRU(...)` |
...
| `sever.py` | 1-12 | 新增模块级文档字符串，描述自定义亮点 |
...
| `index.html` | 1-15 | 新增 HTML 头部文档注释，描述视觉亮点 |
| `index.html` | 19 | 添加 D3.js v7 CDN 引用注释 |
...
- `index.html` 需要网络连接以加载 D3.js CDN（首次加载后浏览器会缓存）
- 训练脚本需要本地存在 MIT-BIH 数据文件（`100.dat` + `100.hea`）
- 推荐使用虚拟环境运行，避免依赖冲突
